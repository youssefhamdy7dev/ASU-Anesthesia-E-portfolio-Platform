...
class ViewController extends Controller
{
    ...
    {
        $data = [];
        $anestheticCase = Anesthetic::where('pending', 0)->where('residentID', auth()->user()->id)->get();
    ...
    {
        $data = [];
        $intensiveCase = Intensive::where('pending', 0)->where('residentID', auth()->user()->id)->get();
    ...
    {
        $data = [];
        $procedureCase = Procedure::where('pending', 0)->where('residentID', auth()->user()->id)->get();
    ...
    {
        $rotations = Rotation::where('pending', 0)->where('residentID', auth()->user()->id)->get();
    ...
    public function print()
    {
        ...
        $pcData = [];
        $rData = [];
        $resident = Resident::where('id', auth()->user()->id)->select('NID', 'name', 'gender', 'email', 'birthdate', 'residencyYear')->first();

        $ac = Anesthetic::where('pending', 0)->where('residentID', auth()->user()->id)->get();
        if (count($ac) <= 0)
            goto Skip1;
        foreach ($ac as $case) {
        ...
        Skip1:
        $ic = Intensive::where('pending', 0)->where('residentID', auth()->user()->id)->get();
        if (count($ic) <= 0)
            goto Skip2;
        foreach ($ic as $case) {
        ...
        Skip2:
        $pc = Procedure::where('pending', 0)->where('residentID', auth()->user()->id)->get();
        if (count($pc) <= 0)
            goto Skip3;
        foreach ($pc as $case) {
            $facility = Facility::where('id', $case->facilityID)->select('name')->first();
        ...
        Skip3:
        $r = Rotation::where('pending', 0)->where('residentID', auth()->user()->id)->select('name', 'startDate', 'endDate', 'stage')->get();
        if (count($r) <= 0)
            goto LastSkip;
